			...
			<div class="col-md-12">
				...
				<div class="cart">
                    <table class="table table-responsive table-hover table-bordered">
                        ...
                        $i=0;
                        foreach($_SESSION['cart_j_name'] as $key => $value) 
                        {
                            $i++;
                            $arr_cart_j_name[$i] = $value;
                        }

                        $i=0;
                        foreach($_SESSION['cart_g_name'] as $key => $value) 
                        {
                            $i++;
                            $arr_cart_g_name[$i] = $value;
                        }
                        ...
                        <tr>
                            ...
                            <?php 
                            $statement = $pdo->prepare("SELECT * FROM tbl_product WHERE p_id=? AND p_product_type='Gem' AND p_product_type='Jewellery'");
                            {
                                ?>
							<td>
                                <?php echo $arr_cart_g_name[$i]; ?>
                                <?php echo $arr_cart_j_name[$i]; ?>
                            </td>
                            <?php
                            }
                            ?>
                            <!-- <td><?php echo $arr_cart_p_name[$i]; ?></td> -->
                            <!-- <td><?php echo $arr_cart_j_name[$i]; ?></td> -->
                            <!-- <?php
                                $statement = $pdo->prepare("SELECT * FROM tbl_product");
                                $statement->execute();
                                $result = $statement->fetchAll(PDO::FETCH_ASSOC);
                                foreach ($result as $row) {
                                    $p_product_type = $row['p_product_type'];
                                    $p_id = $row['p_id'];
                                    if($p_product_type=='Jewellery'&& $p_id==['p_id']) {
                                        ?>
                                            <td><?php echo $arr_cart_j_name[$i]; ?></td>
                                        <?php
                                        }
                                            ...
                                            ?>
                                                <td><?php echo $arr_cart_g_name[$i]; ?></td>
                                            <?php
                                            }
                                }
                            ?> -->
                            ...
                            <td>
                                ...
                                <input type="hidden" name="product_name[]" value="<?php echo $arr_cart_j_name[$i]; ?>">
